...
class AttendanceCacheService {
  ...
  // Cache entry class to track validity
  class _CacheEntry {
    final dynamic data;
    ...
    bool get isValid => DateTime.now().difference(timestamp) < _cacheValidity;
  }
  ...
  Future<T?> getCachedData<T>(String key, String type) async {
    ...
    if (memoryEntry != null && memoryEntry.isValid && memoryEntry.type == type) {
  ...
  Future<void> cacheClassSchedule(String classId, List<Map<String, dynamic>> schedule) async {
    await cacheData('schedule_$classId', schedule, 'schedule');
  }

  Future<List<Map<String, dynamic>>?> getCachedClassSchedule(String classId) async {
    return await getCachedData<List<Map<String, dynamic>>>('schedule_$classId', 'schedule');
  }
  ...
  Future<Map<String, dynamic>?> getCachedStudentAttendanceStatus(
    ...
    return await getCachedData<Map<String, dynamic>>('status_${classId}_$studentId', 'status');
  }

  // Cache active sessions
  Future<void> cacheActiveSessions(List<AttendanceModel> sessions) async {
    await cacheData('active_sessions', sessions.map((s) => s.toJson()).toList(), 'sessions');
  }

  Future<List<AttendanceModel>?> getCachedActiveSessions() async {
    final sessions = await getCachedData<List<Map<String, dynamic>>>('active_sessions', 'sessions');
  ...
  Future<void> cacheClassStats(String classId, Map<String, dynamic> stats) async {
